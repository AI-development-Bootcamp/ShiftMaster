...
async function resetDatabase() {
  ...
  if (!connectionString) {
    logDbError(
      ...
      new Error('SUPABASE_URL or DATABASE_URL is not defined')
    );
    process.exit(1);
  }
  ...
  if (process.env.ALLOW_SCHEMA_RESET !== 'true') {
    logDbError(
      ...
      new Error('ALLOW_SCHEMA_RESET environment variable must be set to "true"')
    );
    process.exit(1);
  }
  ...
  if (
    ...
    process.env.FORCE_RESET_PROD !== 'true'
  ) {
    logDbError(
      'Database reset aborted',
      new Error(
        'Cannot reset database in production without FORCE_RESET_PROD="true"'
      )
    );
    process.exit(1);
  }
